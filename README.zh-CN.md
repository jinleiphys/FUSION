...
**上面两条命令都要连 GitHub，在国内网络下通常会失败。** 2026-08-13 在校园网实测：`github.com` 六次连接零成功，`git clone` 等了 132 秒后报 `Failed to connect to github.com port 443`，下载二进制的那条也超时。它时通时不通，所以过一小时再试可能就好了；**等一两分钟然后报错是正常症状，不是仓库坏了**。如果你是这个情况，这两步需要代理。代理只在这两步需要：装完之后不再依赖 GitHub，各个物理程序是首次使用时从它们各自的上游拉取，文献库本来就在克隆里。不需要代理的镜像还没做，记在 TODO.md 里。
