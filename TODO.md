...
- [ ] **Fission/statistical row: openness problem.** GEMINI++'s two known distribution URLs (chemistry.wustl.edu/~rc/gemini++/, sourceforge) both return 404, so it may fail the publicly-obtainable rule outright. GEF's site responds; check its licence and distribution terms before committing to it. This row has no confirmed-obtainable candidate yet, which makes it the riskiest of the four
