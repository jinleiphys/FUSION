...
- [ ] **Fission/statistical row: GEMINI++ DROPPED (user, 2026-07-21), GEF is the remaining candidate.** GEMINI++'s two known distribution URLs (chemistry.wustl.edu/~rc/gemini++/, sourceforge) both return 404, so it fails the publicly-obtainable rule and is removed from the catalog rather than left as a maybe. Next: check GEF's licence and distribution terms. If GEF does not clear either, the fallback is to ask whether TALYS's fission channel can carry this row, which is a code-axis design change and must be recorded in the paper README
