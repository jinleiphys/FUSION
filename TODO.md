...
- [ ] **AZURE2 next actions, in order (resume here).** (1) Build the ¹⁶O(p,γ)¹⁷F `.azr` from PRC 81, 045805 Table V: 5 levels, `Eλ` = 0.00 / 495 / 3103 / 3859 / 4711 keV, `C_d5/2 = 1.05` and `C_s1/2 = 80.7 fm^-1/2` for the two bound states, `γp` = 0.1415 / 0.2456 / 0.7519, plus the `γγ0` and `γγ1` internal widths; check against Fig. 15. (2) Build ¹⁴N(p,γ)¹⁵O and compare **digits** against Table IV (0.28 / 0.01 / 0.10 / 0.12 / 1.30, total **1.81 keV b**). (3) Then `SKILL.md`, `references/` from the AZURE2 manual (never from the README or memory), `run_azure2.sh` with positive-assertion checks, `verify_azure2.sh`, and the mandatory Codex adversarial pass. Local copies of the source PDFs are in `~/Downloads` (`PhysRevC.81.045805-2.pdf`, `deBoer_2015.pdf`, `TALENT6-JD-Lecture2.pdf`); extracted text is in the session scratchpad, which is not durable, so re-extract if needed
- [ ] **GEF skill (fission row), not started.** Runs on heliumx via the shipped `GEF64` (do not fight FreeBASIC on macOS). Batch mode needs a `file.in` listing the deck. **Two mandatory guards**: clear `ctl/` (the `done.ctl` memo silently skips a completed case at exit 0) and remove any stale `Fitpar.dat` (it overrides shipped defaults). Deck format: line 3 is the options line, omitting it shifts the Z,A line. Look for an L2 anchor in the 115-page Nucl. Data Sheets paper before settling for tier 2. Verified working case: `98,252,"GS"` gives nu-bar 3.8207
...
- [ ] **devlog.md is oversized for an auto-imported file (16 KB, target ~5 KB).** Pre-2026-07-21 entries were archived on 2026-07-21, but today's four entries alone exceed the budget and they overlap heavily in theme (false-success modes, guard bugs written against the wrong operand, verification philosophy). Consolidate them into one shorter entry that keeps every distinct mechanism and drops the repeated narration [user decision: which lessons must survive verbatim]
