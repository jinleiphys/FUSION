...
- [ ] **AZURE2 remaining: the 6.79 MeV transition of 14N(p,g)15O.** The full Table IV case is NOT buildable (see the 2026-07-22 Key decision), but this one transition is 72% of the total (S(0) = 1.30 keV b of 1.81), is external-capture dominated with a single ANC C_s3/2 = 4.86 fm^-1/2, and the 0.259 resonance is explicitly "added incoherently" so there is no sign ambiguity. Channel radius for this reaction is **5.5 fm**, not the 5.0 used for 17F; Sp(15O) = 7.2971 MeV is external (TUNL), the paper never states it
