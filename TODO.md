...
- [ ] **Semantic layer (L3) weekend auto-finish** (off-peak launcher run_full_relations.sh, self-looping, PID 12211): as of 2026-07-17, 51,197/54,378 citing papers classified, 716k typed edges (473k background dropped, ~242k kept, 34%); contrasts 2.4% (healthy). Only 3,181 papers left (daytime DeepSeek throttled, so left to tonight's off-peak window). Launcher then auto-injects `## Related work` across ~51k pages. MONDAY: Claude does final QC (type distribution, contrasts large-sample evidence spot-check, Related work section on real pages) + commit + push. Design [semantic-layer-design.md](semantic-layer-design.md). NOTE: launcher is a nohup process (not launchd), so a weekend machine reboot would stop it; empirically it has survived since 2026-07-15
