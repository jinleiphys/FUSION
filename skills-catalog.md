...
- [x] **SFRESCO** (Thompson; ships inside the FRESCO distribution, GPL-3.0, same repo) DONE: `skills/sfresco/` (2026-07-30). The MINUIT chi-squared **fitting front end** of FRESCO: `.search` file authoring, search variables (potential parameters, spectroscopic amplitudes, R-matrix energies and widths, dataset normalisations), batch MINUIT sessions, error and correlation reporting. **Not a new code**, so it does not consume a per-code row or a subfield area; it is the companion that makes the FRESCO row cover parameter *estimation* as well as forward calculation. The `sfresco` binary contains FRESCO, so no separate build.
  - **PUBLISHED-WORK ANCHOR (the 2026-07-24 pass criterion), L2 exact:** `examples/p-cd-manual.*` transcribes Boxes 7 and 8 of *FRESCO: getting started* section 4 (p + 112Cd elastic at 27.9 MeV, 12 points, start values r0 = 1.0, V = 50, W = 5, W_d = 10) and reproduces **every digit the manual prints**: V = 52.5280 vs 52.53, r0 = 1.17958 vs 1.179, W = 3.46041 vs 3.46, W_d = 7.42937 vs 7.43, chi2/N = 2.1910 vs 2.19. The 15% displaced starting r0 makes this a fit, not a re-echo of the deck.
  - Two further verified cases: the distribution's own 28-point `test/ss.search` (chi2/N 20.5232 -> 8.935, reference `test/Outputs/ss-fit.plot` matched to 4-5 digits) and `nalpha` phase shifts, 3 datasets, 78 points (26021.14 -> 392.80, corr = -0.996, a deliberately degenerate pair). Plus a **closure test** with a known exact answer (`p90zr-closure`: pseudo-data from a KD02 p + 90Zr deck, V and W_d started 14% and 32% away, recovered to 5 digits) as the environment/regression gate.
  - Verified on **FRES 3.4** (the 2023 binary in `~/bin`) and on **FRES 3.5** (fresh build from the repo); chi-squared and fitted parameters agree, MINUIT error columns differ in the last digits. Note 3.5 accepts `type=9` ANC data, 3.4 rejects it with `Unrecognised data type 9`.
  - Cross-validated against the FRESCO source (`sfresco.f`, `frxx0.f`, `frxx1.f`, `frxx2.f`, `frxx5.f`, `frxx17.f`) by Codex in three passes: 2 documentation claims corrected (`pline` also counts `&step` records for TYPE 12-17; the CC-iteration 10000 penalty is unconditional while the bound-state one is gated by `number_calls>5`) and 14 script bugs fixed. Traps documented and reproduced: `p` is declared `p(0:8)` so a bare `p=` shifts every column; a comment inside a data block silently truncates the dataset (29 points read as 6); a row with the wrong number of columns makes the list-directed read swallow the next row; `abserr` defaults to *relative* errors.
  - Fixed in the sibling skill while doing this: `install_fresco.sh` detected only `fresco` and reported success when `sfresco` was missing, so a machine with a hand-built `fresco` could never auto-provision `sfresco`. Detection now requires both. Cold-start install verified end to end from an empty bin dir (clone, build, then the closure fit passes).
