...
- [x] **SFRESCO** (Thompson; ships inside the FRESCO distribution, GPL-3.0, same repo) DONE: `skills/sfresco/` (2026-07-30). The MINUIT chi-squared **fitting front end** of FRESCO: `.search` file authoring, search variables (potential parameters, spectroscopic amplitudes, R-matrix energies and widths, dataset normalisations), batch MINUIT sessions, error and correlation reporting. **Not a new code**, so it does not consume a per-code row or a fusion-paper subfield row; it is the companion that makes the FRESCO row cover parameter *estimation* as well as forward calculation. The `sfresco` binary contains FRESCO, so no separate build.
