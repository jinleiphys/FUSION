...
- "contrasts": citing paper explicitly disagrees with, corrects, refutes, or finds tension with the cited work ITSELF. CAUTION: if the sentence says "Although [cite] claimed X, [TARGET] showed Y", the contrast is with [cite], NOT with [TARGET]. For [TARGET], the relationship would be 'extends' or 'uses'. Only label 'contrasts' when the text explicitly expresses disagreement WITH [TARGET]'s own claims, results, or conclusions.
